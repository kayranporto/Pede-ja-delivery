...
(() => {
  ...
  if(!document.querySelector(".skip-link")){const skip=document.createElement("a");skip.className="skip-link";skip.href="#conteudoPrincipal";skip.textContent="Pular para o conteúdo";skip.style.position="fixed";skip.style.left="-10000px";skip.style.top="8px";skip.style.zIndex="20000";skip.style.width="1px";skip.style.height="1px";skip.style.overflow="hidden";skip.style.padding="1px";skip.style.clipPath="inset(50%)";skip.addEventListener("focus",()=>{skip.style.left="8px";skip.style.width="auto";skip.style.height="auto";skip.style.overflow="visible";skip.style.padding="10px 14px";skip.style.clipPath="none"});skip.addEventListener("blur",()=>{skip.style.left="-10000px";skip.style.width="1px";skip.style.height="1px";skip.style.overflow="hidden";skip.style.padding="1px";skip.style.clipPath="inset(50%)"});skip.onclick=e=>{e.preventDefault();const main=document.querySelector("main,.page-main");if(!main)return;main.id=main.id||"conteudoPrincipal";main.setAttribute("tabindex","-1");main.focus()};document.body.prepend(skip)}
})();
