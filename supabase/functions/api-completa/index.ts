...
async function adminAction(ctx: RouteContext, body: Json) {
  ...
  if (action === "restaurante_atualizar") return rpc(ctx.db,ctx.request,"admin_atualizar_restaurante",body);
  if (action === "restaurante_excluir") return rpc(ctx.db,ctx.request,"admin_excluir_restaurante",{p_empresa_id:String(body.empresa_id||""),p_confirmacao:str(body.confirmacao,200)});
